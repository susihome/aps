 ...
 */
function getSpringFestivalDates(year: number): string[] {
 ...
 */
function getQingmingFestivalDates(year: number): string[] {
  // 清明节通常在4月4日、5日或6日
  // 这里使用4月4日-6日作为通用假期
 ...
 * 生成劳动节日期（5月1日-5日）
 */
function getLaborDayDates(year: number): string[] {
 ...
 */
function getDragonBoatFestivalDates(year: number): string[] {
  const dragonBoatFestivals: Record<number, string[]> = {
    2024: ['2024-06-10'],
 ...
 */
function getMidAutumnFestivalDates(year: number): string[] {
  const midAutumnFestivals: Record<number, string[]> = {
 ...
 * 生成国庆节日期（10月1日-7日）
 */
function getNationalDayDates(year: number): string[] {
 ...
 * 节假日模板配置
 */
export const holidayTemplates: HolidayTemplate[] = [
  {
    key: 'spring',
    name: '春节',
 ...
 * @returns 日期数组
 */
export function getHolidayDates(template: string, year: number): string[] {
 ...
 * 获取支持的年份范围
 */
export const SUPPORTED_YEARS = [2024, 2025, 2026, 2027, 2028] as const
